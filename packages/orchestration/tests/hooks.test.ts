...
describe("withHooks", () => {
  ...
  it("ask 仅在 owner 显式批准后执行一次", async () => {
    ...
    const wrapped = withHooks({ id: "test.tool", execute: exec }, {
      runner,
    ...
    const first = (await wrapped.execute!({ x: 1 })) as {
      requestId: string;
      requiresApproval: boolean;
      message: string;
    };
    expect(first.requiresApproval).toBe(true);
    expect(first.message).toMatch(/latest language/i);
    expect(exec).not.toHaveBeenCalled();
    expect(store.list("user:alice")).toHaveLength(1);

    expect(store.respond(first.requestId, "allow", "user:alice")).toBe(true);
    expect(await wrapped.execute!({ x: 1 })).toEqual({ ran: true });
    expect(exec).toHaveBeenCalledOnce();

    const replay = (await wrapped.execute!({ x: 1 })) as { requiresApproval: boolean };
    expect(replay.requiresApproval).toBe(true);
    expect(exec).toHaveBeenCalledOnce();
    store.clearAll();
  });

  it("ask 批准绑定稳定 input digest", async () => {
    const { PendingApprovalsStore } = await import("../src/permissions/pending.js");
    const store = new PendingApprovalsStore();
    const exec = vi.fn().mockResolvedValue({ ran: true });
    const wrapped = withHooks({ id: "test.tool", execute: exec }, {
    ...
    const first = (await wrapped.execute!({ x: 1 })) as { requestId: string };
    store.respond(first.requestId, "allow", "user:alice");

    const changed = (await wrapped.execute!({ x: 2 })) as { requiresApproval: boolean };
    expect(changed.requiresApproval).toBe(true);
    expect(exec).not.toHaveBeenCalled();
    expect(await wrapped.execute!({ x: 1 })).toEqual({ ran: true });
    store.clearAll();
  });
...
describe("defaultGetAuthSub", () => {
  it("只读取 identity middleware 注入的已认证主体", () => {
    const rc = new Map<string, unknown>([[AUTH_SUB_KEY, "user-42"]]);
    expect(defaultGetAuthSub({ requestContext: rc })).toBe("user-42");
    expect(defaultGetAuthSub({ requestContext: new Map() })).toBeUndefined();
  });
});

describe("defaultGetSessionId", () => {
  it("owner 与稳定 thread 分别提取，不能互相替代", () => {
    const rc = new Map<string, unknown>([[AUTH_SUB_KEY, "user-42"]]);
    const ctx = { requestContext: rc, agent: { threadId: "t-1" } };
    expect(defaultGetAuthSub(ctx)).toBe("user-42");
    expect(defaultGetSessionId(ctx)).toBe("t-1");
  });

  it("authSub 缺失不影响稳定 thread 提取", () => {
  ...
  it("does NOT fall back to per-turn runId", () => {
    // runId 每 user-turn 变化，不能作为稳定审批 thread；缺失时必须 fail closed。
